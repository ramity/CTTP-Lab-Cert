...
<html>
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="<?php generate_url('css/display.css');?>"/>
        ...
        <div class="display">
          ...
          foreach($o=scandir(generate_path("bend/file_storage/AMRL Equipment Calibrations/")) as $oitem)
          {
            if(strpos($oitem,'Equipment Calibrations')!==false)
            {
              ...
              echo '<a href="' . generate_url('amrl/index.php?year=' . $bits[0], false) . '">';
                    ...
                    foreach($list as $data)
                    {
                      try
                      {
                        ...
                        if(!empty($sr))
                        {
                          ...
                          echo '<td>';
                          echo '<a href="' . generate_url('view.php?viewitem=' . $main_id . '&table=' . $table, false) . '">View</a>';
                          echo '<a href="' . generate_url('view.php?edititem=' . $main_id . '&table=' . $table, false) . '">Edit</a>';

                          if($sr[0]['display'])
                            echo '<a href="' . generate_url('view.php?removeitem=' . $main_id . '&table=' . $table, false) . '">Remove</a>';
                          else
                            echo '<a href="' . generate_url('view.php?undoitem=' . $main_id . '&table=' . $table, false) . '">Undo</a>';
